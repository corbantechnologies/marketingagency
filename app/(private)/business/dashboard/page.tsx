      ...
      <div className="bg-white border border-zinc-200 rounded-lg p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded bg-purple-50 border border-purple-200 text-[11px] font-semibold text-[#581c87] mb-2">
            Default Workspace: {primaryBusiness ? primaryBusiness.name : "Active"}
          </div>
          <h1 className="text-xl font-bold text-zinc-900 tracking-tight">
            Bulk SMS &amp; Email Marketing Dashboard
          </h1>
          <p className="text-xs text-zinc-600 mt-1">
            Account: {session?.user?.email} &bull; Member Code: {session?.user?.member_code || session?.user?.code || "MA26001"}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link
            ...
            className="py-2 px-3.5 bg-[#581c87] hover:bg-[#4a1572] text-white text-xs font-semibold rounded transition-colors flex items-center gap-1.5 shadow-xs"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-zinc-200 rounded-lg p-5 shadow-xs">
          ...
          <div className="text-[11px] text-zinc-500 mt-2 flex items-center gap-1">
            <span className="text-emerald-600 font-semibold">&bull; Free trial</span> balance ready for test broadcasts
          </div>
        </div>
        ...
        <div className="bg-white border border-zinc-200 rounded-lg p-5 shadow-xs">
          ...
          <div className="text-[11px] text-zinc-500 mt-2 flex items-center gap-1">
            <span className="text-purple-600 font-semibold">&bull; Shared route</span> available immediately
          </div>
        </div>
        ...
        <div className="lg:col-span-8 bg-white border border-zinc-200 rounded-lg p-6 shadow-xs">
          ...
          {isBusinessesLoading ? (
            ...
            <div className="divide-y divide-zinc-100">
              {businesses.map((biz) => (
                <div key={biz.id || biz.reference} className="py-3.5 flex items-center justify-between text-xs">
                  <div>
                    <div className="font-semibold text-zinc-900 text-sm">{biz.name}</div>
                    <div className="text-zinc-500 mt-0.5">
                      Email: {biz.email || "—"} &bull; Phone: {biz.phone || "—"} &bull; Ref: {biz.reference}
                    </div>
                  </div>
          ...
          <div className="space-y-3 text-xs">
            <div className="p-3 bg-zinc-50 rounded border border-zinc-200">
              ...
              <div className="text-zinc-600 leading-relaxed">
                Send a live test SMS to your phone to check carrier latency and delivery report speed.
              </div>
            </div>
