...
export default function AdminRateCardPage() {
  ...
  const [waCost, setWaCost] = useState<number>(0.50);
  ...
  const [waMarkup, setWaMarkup] = useState<number>(160);
  ...
  useEffect(() => {
    if (data) {
      if (data.wholesale_benchmarks) {
        ...
        if (data.wholesale_benchmarks.whatsapp_base_kes !== undefined) {
          setWaCost(data.wholesale_benchmarks.whatsapp_base_kes);
        }
      }
      if (data.markup_targets) {
        ...
        if (data.markup_targets.whatsapp_markup_pct !== undefined) {
          setWaMarkup(data.markup_targets.whatsapp_markup_pct);
        }
      }
    }
  }, [data]);
  ...
  // Trigger initial simulation when data loads or channel changes
  useEffect(() => {
    if (data) {
  ...
  }, [data, simChannel]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleChannelChange = (newChannel: "SMS" | "WHATSAPP") => {
    setSimChannel(newChannel);
    const newRetail = newChannel === "WHATSAPP" ? 1.20 : 0.80;
    setSimRetailRate(newRetail);
    simulateMutation.mutate({
      channel: newChannel,
      volume: simVolume,
      retail_rate_kes: newRetail,
      safaricom_pct: simSafPct,
      airtel_pct: simAirPct,
      telkom_pct: simTelPct,
    });
    ...
    try {
      await saveMutation.mutateAsync({
        wholesale: {
          ...
          whatsapp_base_kes: waCost,
        },
        markup_targets: {
          standard_markup_pct: standardMarkup,
          volume_markup_pct: volumeMarkup,
          enterprise_markup_pct: enterpriseMarkup,
          whatsapp_markup_pct: waMarkup,
        },
      });
      toast.success("Carrier & WhatsApp benchmarks updated successfully!");
      refetch();
    } catch (err: any) {
    ...
    <div className="space-y-8 pb-16">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 border border-emerald-200">
              Omnichannel Commercial Intelligence
            </span>
            <span className="text-xs text-zinc-500">
              Safaricom &bull; Airtel &bull; Meta WhatsApp Cloud API Margins
            </span>
          </div>
          <h1 className="text-2xl font-bold text-zinc-900 mt-1">
            Omnichannel Rate Cards &amp; Auto-Markup Engine
          </h1>
          <p className="text-sm text-zinc-500 mt-0.5">
            Configure wholesale carrier and Meta benchmarks, audit commercial plan gross margins, and simulate campaign profitability.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="p-4 bg-white rounded-xl border border-zinc-200 shadow-2xs">
          ...
          <p className="text-[11px] text-zinc-500 mt-1">Wholesale buy cost / SMS</p>
        </div>
        ...
        <div className="p-4 bg-white rounded-xl border border-zinc-200 shadow-2xs">
          <div className="flex items-center justify-between text-xs font-medium text-zinc-500">
            ...
            <span className="px-1.5 py-0.5 rounded bg-red-50 text-red-700 font-bold text-[10px]">
              25% SMS
            </span>
          </div>
          <div className="mt-2 text-2xl font-extrabold text-zinc-900 tracking-tight">
        ...
        <div className="p-4 bg-white rounded-xl border border-emerald-200 bg-emerald-50/20 shadow-2xs">
          ...
          <div className="mt-2 text-2xl font-extrabold text-emerald-950 tracking-tight">
            KES {waCost.toFixed(4)}
          </div>
          <p className="text-[11px] text-emerald-700 mt-1">Wholesale fee / conversation</p>
        </div>
        ...
        <div className="p-4 bg-gradient-to-br from-purple-900 to-indigo-950 text-white rounded-xl shadow-xs border border-purple-800">
          <div className="flex items-center justify-between text-xs font-medium text-purple-200">
            <span>Blended SMS Floor</span>
            <span className="px-1.5 py-0.5 rounded bg-purple-800/80 text-purple-100 font-bold text-[10px]">
              WEIGHTED
            </span>
          </div>
      ...
      {/* SECTION 1: Carrier & WhatsApp Wholesale Benchmarks & Markup Config */}
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-2xs overflow-hidden">
        <div className="px-6 py-4 border-b border-zinc-100 bg-zinc-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <h2 className="text-base font-bold text-zinc-900">Carrier &amp; Meta Wholesale Benchmarks</h2>
            <p className="text-xs text-zinc-500">
              Set the true per-message wholesale purchase prices contracted with Advanta, Africa&apos;s Talking, and Meta Cloud API.
            </p>
          </div>
          <span className="text-xs font-semibold px-2.5 py-1 rounded-md bg-zinc-200 text-zinc-700">
            Omnichannel Compliant &bull; Cache Persisted
          </span>
        </div>
        ...
        <form onSubmit={handleSaveBenchmarks} className="p-6 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* Meta WhatsApp Input */}
            <div className="space-y-1.5 p-4 rounded-xl border border-emerald-200 bg-emerald-50/30">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-emerald-900 flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-emerald-600 inline-block" />
                  Meta WhatsApp (KES)
                </label>
                <span className="text-[11px] text-emerald-600 font-medium">Std: 0.5000</span>
              </div>
              <input
                type="number"
                step="0.0001"
                ...
                className="w-full px-3 py-2 text-sm font-semibold rounded-lg border border-emerald-300 bg-white text-zinc-900 focus:ring-2 focus:ring-emerald-500 focus:outline-hidden"
              />
              <p className="text-[11px] text-emerald-700">Meta Cloud API marketing conversation fee (~$0.0038).</p>
            </div>
          </div>
          ...
          <div className="border-t border-zinc-100 pt-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <label className="text-xs text-zinc-600 block mb-1">Standard SME SMS Markup</label>
              ...
              <div>
                <label className="text-xs text-zinc-600 block mb-1">High-Volume Tier SMS Markup</label>
                <div className="relative">
                  <input
                    type="number"
                    min="10"
              ...
              <div>
                <label className="text-xs text-zinc-600 block mb-1">Enterprise Tier SMS Markup</label>
                <div className="relative">
                  <input
                    type="number"
                    min="10"
              ...
              <div>
                <label className="text-xs text-emerald-800 font-semibold block mb-1">WhatsApp Marketing Markup</label>
                <div className="relative">
                  <input
                    type="number"
                    min="10"
                    max="500"
                    value={waMarkup}
                    onChange={(e) => setWaMarkup(parseInt(e.target.value) || 0)}
                    className="w-full px-3 py-2 text-sm font-semibold rounded-lg border border-emerald-300 bg-emerald-50/30 text-emerald-950 pr-8"
                  />
                  <span className="absolute right-3 top-2.5 text-xs text-emerald-600 font-bold">%</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-2xs overflow-hidden">
        <div className="px-6 py-4 border-b border-zinc-100 bg-zinc-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-zinc-500">
              Audit actual gross margins across active commercial plans for both Tier-1 Bulk SMS and Meta WhatsApp Cloud API.
            </p>
          </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-zinc-100/70 border-b border-zinc-200 text-zinc-600 font-bold uppercase tracking-wider text-[11px]">
              <tr>
                ...
                <th className="px-3 py-3.5">SMS Rate</th>
                <th className="px-3 py-3.5 text-emerald-800">WhatsApp Rate</th>
                <th className="px-3 py-3.5">SMS Margin</th>
                <th className="px-3 py-3.5 text-emerald-800">WhatsApp Margin</th>
                <th className="px-4 py-3.5 text-right">Profit / 10k SMS</th>
                <th className="px-6 py-3.5 text-right text-emerald-800">Profit / 10k WA</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100 text-zinc-700">
              {isLoading ? (
                ...
                data.plans.map((p) => {
                  ...
                  const waRate = p.whatsapp_rate_kes ?? (p.sms_rate_kes * 2);
                  const waMargin = p.whatsapp_margin_pct ?? (waRate > 0 ? (((waRate - waCost) / waRate) * 100) : 0);
                  const waProfit = p.whatsapp_profit_per_10k_kes ?? ((waRate - waCost) * 10000);
                  ...
                  return (
                    <tr key={p.reference} className="hover:bg-zinc-50/80 transition-colors">
                      ...
                      <td className="px-3 py-4 font-bold text-zinc-900 whitespace-nowrap">
                        KES {p.sms_rate_kes.toFixed(2)}
                      </td>

                      <td className="px-3 py-4 font-bold text-emerald-900 whitespace-nowrap">
                        KES {waRate.toFixed(2)} <span className="text-[10px] text-emerald-600 font-normal">(2 cr)</span>
                      </td>

                      <td className="px-3 py-4">
                        <span className={`inline-block px-2 py-0.5 rounded-full border text-[11px] font-bold ${getBadge(p.blended_margin_pct)}`}>
                          {p.blended_margin_pct.toFixed(1)}%
                        </span>
                      </td>

                      <td className="px-3 py-4">
                        <span className={`inline-block px-2 py-0.5 rounded-full border text-[11px] font-bold ${getBadge(waMargin)}`}>
                          {waMargin.toFixed(1)}%
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-4 text-right">
                        ...
                        <span className="text-[10px] text-zinc-400">Per 10k SMS</span>
                      </td>

                      <td className="px-6 py-4 text-right">
                        <div className="font-extrabold text-emerald-700 text-xs">
                          +KES {Math.round(waProfit).toLocaleString()}
                        </div>
                        <span className="text-[10px] text-emerald-600 font-medium">Per 10k WA</span>
                      </td>
                    </tr>
                  );
                })
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-2xs overflow-hidden">
        <div className="px-6 py-4 border-b border-zinc-100 bg-purple-50/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
          ...
          <div className="inline-flex items-center p-1 bg-white rounded-xl border border-purple-200 shadow-2xs self-start sm:self-center">
            <button
              type="button"
        ...
        <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-6 space-y-5">
            ...
            <div>
              <div className="flex justify-between items-center text-xs font-semibold text-zinc-700 mb-1">
                <span>Campaign {simChannel === "WHATSAPP" ? "WhatsApp Messages" : "SMS Volume"}</span>
                <span className={`text-sm font-extrabold ${simChannel === "WHATSAPP" ? "text-emerald-800" : "text-purple-900"}`}>
                  {simVolume.toLocaleString()} {simChannel === "WHATSAPP" ? "Flyers" : "SMS"}
                </span>
              </div>
              <input
                type="range"
                ...
                className={`w-full cursor-pointer ${simChannel === "WHATSAPP" ? "accent-emerald-600" : "accent-purple-700"}`}
              />
            ...
            <div>
              <div className="flex justify-between items-center text-xs font-semibold text-zinc-700 mb-1">
                <span>Client Quoted Retail Rate (KES / {simChannel === "WHATSAPP" ? "Message" : "SMS"})</span>
                <span className={`text-sm font-extrabold ${simChannel === "WHATSAPP" ? "text-emerald-800" : "text-purple-900"}`}>
              ...
              <input
                type="range"
                min={simChannel === "WHATSAPP" ? "0.60" : "0.40"}
                max={simChannel === "WHATSAPP" ? "3.00" : "2.00"}
                step="0.05"
                value={simRetailRate}
                onChange={(e) => {
              ...
              />
              <div className="flex justify-between text-[10px] text-zinc-400 mt-1">
                <span>{simChannel === "WHATSAPP" ? "KES 0.60 (Wholesale)" : "KES 0.40 (Heavy Tier)"}</span>
                <span>{simChannel === "WHATSAPP" ? "KES 1.20 (Standard)" : "KES 0.80 (Standard)"}</span>
                <span>{simChannel === "WHATSAPP" ? "KES 3.00 (Premium)" : "KES 2.00 (Ad-hoc)"}</span>
              </div>
            </div>

            {/* If SMS: Show Telco Sliders; If WhatsApp: Show Meta Route Details */}
            {simChannel === "SMS" ? (
              ...
              <div className="p-4 rounded-xl border border-emerald-200 bg-emerald-50/40 space-y-3">
                <span className="text-xs font-bold uppercase tracking-wider text-emerald-900 block">
                  Meta WhatsApp Cloud API Economics
                </span>
                <p className="text-xs text-emerald-800 leading-relaxed">
                  WhatsApp charges per marketing conversation (24-hour window) rather than per 160-character part. With zero telecom SMS segmentation penalties, multi-part flyer dispatches retain 100% margin.
                </p>
                <div className="grid grid-cols-2 gap-2 text-xs pt-1">
                  <div className="p-2.5 rounded bg-white border border-emerald-200">
                    <span className="text-zinc-500 block text-[10px]">Wholesale Meta Fee</span>
                    <strong className="text-emerald-950 font-bold">KES {waCost.toFixed(2)} / conv</strong>
                  </div>
                  <div className="p-2.5 rounded bg-white border border-emerald-200">
                    <span className="text-zinc-500 block text-[10px]">Client Unified Credits</span>
                    <strong className="text-emerald-950 font-bold">2 Credits / Msg</strong>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="lg:col-span-6 flex flex-col justify-between space-y-4">
            ...
            <div className={`p-6 rounded-2xl text-white shadow-lg space-y-6 ${
              simChannel === "WHATSAPP"
                ? "bg-gradient-to-br from-emerald-950 via-zinc-900 to-emerald-950 border border-emerald-800/60"
                : "bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900"
            }`}>
              <div className="flex items-center justify-between border-b border-zinc-700/60 pb-3">
                <span className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1.5">
                  <span>{simChannel === "WHATSAPP" ? "💬 WhatsApp" : "📱 SMS"} Simulation Results</span>
                </span>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <span className="text-xs text-zinc-400 block">Wholesale Cost</span>
                ...
                {simChannel === "WHATSAPP" ? (
                  <div className="flex items-center justify-between py-1">
                    <span className="flex items-center gap-1.5 text-zinc-300">
                      ...
                      Meta Cloud API ({simVolume.toLocaleString()} Flyers @ KES {waCost.toFixed(2)})
                    </span>
