...
import { useCreateAdminUser } from "@/hooks/accounts/actions";

export default function AdminAlphaSignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.email.trim()) {
      setErrorMessage("Please enter your agency staff email address.");
      return;
    }
    if (!formData.phone.trim()) {
      setErrorMessage("Please enter your staff phone number.");
      return;
    }
    ...
    createAdminMutation.mutate(
      ...
      },
      {
        onSuccess: async (data) => {
          toast.success("Agency Administrator provisioned! Redirecting...");
          ...
          try {
            ...
            const redirectTarget = data.redirect_url || "/admin/dashboard";
        ...
        onError: (error: any) => {
          ...
          const msg =
            ...
            resData?.message ||
            error?.message ||
            "Unable to register administrator account.";
          setErrorMessage(msg);
          toast.error(msg);
        },
      }
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-lg p-6 sm:p-8 shadow-xs">
      <div className="mb-6">
        <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded bg-amber-50 border border-amber-200 text-[11px] font-semibold text-amber-800 mb-2">
          Agency Staff &amp; Administrator Registration
        </div>
        <h1 className="text-xl font-semibold text-zinc-900 tracking-tight">
          Create Agency Admin Account
        </h1>
        <p className="text-xs text-zinc-600 mt-1">
          Provision super-admin access for carrier routing, client management, and billing overrides.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            placeholder="admin@ljkmarketingagency.co.ke"
            disabled={isSubmitting}
            required
            autoComplete="email"
            className="w-full px-3.5 py-2.5 bg-white border border-zinc-300 rounded text-sm text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:border-[#581c87] focus:ring-1 focus:ring-[#581c87] transition-colors disabled:bg-zinc-100"
        ...
        <button
          type="submit"
          disabled={isSubmitting}
          className="w-full py-2.5 px-4 bg-zinc-900 hover:bg-zinc-800 active:bg-zinc-950 text-white font-semibold rounded text-sm transition-all duration-150 flex items-center justify-center gap-2 shadow-xs disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer mt-2"
        >
          {isSubmitting ? (
            <>
            ...
            <span>Create Administrator Account &rarr;</span>
          )}
        </button>
      </form>
